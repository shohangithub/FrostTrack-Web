...
export class RecurringChargeManagementService extends BaseService {
  ...
  preview(asOfDate?: string): Observable<IRecurringChargePreview> {
    ...
    return this.http.get<IRecurringChargePreview>(`${this.path}/preview`, {
      params,
    });
  }

  apply(
    request: IRecurringChargeRunRequest,
  ): Observable<IRecurringChargeRunResponse> {
    return this.http.post<IRecurringChargeRunResponse>(
      `${this.path}/apply`,
      request,
    );
  }
  ...
  getHistory(take = 30): Observable<IRecurringChargeRunResponse[]> {
    ...
    return this.http.get<IRecurringChargeRunResponse[]>(
      `${this.path}/history`,
      {
        params,
      },
